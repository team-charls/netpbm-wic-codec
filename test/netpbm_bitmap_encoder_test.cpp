...
[[nodiscard]]
uint32_t compute_stride(const uint32_t width, int32_t bits_per_sample, int32_t component_count) noexcept
{
    uint32_t stride;

    if (bits_per_sample == 2)
    {
        stride = (width + 3) / 4 * component_count;
    }
    else if (bits_per_sample == 4)
    {
        stride = (width + 1) / 2 * component_count;
    }
    else if (bits_per_sample == 8)
    {
        stride = width * component_count;
    }
    else
    {
        stride = width * 2 * component_count;
    }
...
TEST_CLASS(netpbm_bitmap_encoder_test)
{
public:
    TEST_METHOD(GetContainerFormat) // NOLINT
    {
        ...
        Assert::IsTrue(id::container_format_netpbm == container_format);
    }
    ...
    TEST_METHOD(GetContainerFormat_with_nullptr) // NOLINT
    {
        ...
        SUPPRESS_WARNING_NEXT_LINE(6387) // don't pass nullptr
    ...
    //    {
    //        com_ptr<IStream> stream;
    //        check_hresult(SHCreateStreamOnFileEx(destination_filename, STGM_READWRITE | STGM_CREATE |
...
private:
    void encode_monochrome_2_bit(const char* source_filename, const wchar_t* destination_filename) const
    {
...

        {
            ...
            const uint32_t stride = compute_stride(static_cast<uint32_t>(anymap_file.width()), 2, 1);
    ...
    void encode_monochrome_4_bit(const char* source_filename, const wchar_t* destination_filename) const
    {
...

        {
            ...
            const uint32_t stride = compute_stride(static_cast<uint32_t>(anymap_file.width()), 4, 1);
